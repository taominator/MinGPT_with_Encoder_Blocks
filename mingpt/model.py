...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        type_given = (config.model_type is not None) & (config.n_encoder_layer is not None)
        params_given = all([config.n_layer is not None, config.n_head is not None, config.n_embd is not None, config.n_encoder_layer is not None])
        ...
        if type_given:
            ...
            config.merge_from_dict({
                ...
                # GPT-1
                'openai-gpt':   dict(n_layer=12, n_head=12, n_embd=768, n_encoder_layer=config.n_encoder_layer),  # 117M params
                # GPT-2 configs
                'gpt2':         dict(n_layer=12, n_head=12, n_embd=768, n_encoder_layer=config.n_encoder_layer),  # 124M params
                'gpt2-medium':  dict(n_layer=24, n_head=16, n_embd=1024, n_encoder_layer=config.n_encoder_layer), # 350M params
                'gpt2-large':   dict(n_layer=36, n_head=20, n_embd=1280, n_encoder_layer=config.n_encoder_layer), # 774M params
                'gpt2-xl':      dict(n_layer=48, n_head=25, n_embd=1600, n_encoder_layer=config.n_encoder_layer), # 1558M params
                # Gophers
                'gopher-44m':   dict(n_layer=8, n_head=16, n_embd=512, n_encoder_layer=config.n_encoder_layer),
                # (there are a number more...)
                # I made these tiny models up
                'gpt-mini':     dict(n_layer=6, n_head=6, n_embd=192, n_encoder_layer=config.n_encoder_layer),
                'gpt-micro':    dict(n_layer=4, n_head=4, n_embd=128, n_encoder_layer=config.n_encoder_layer),
                'gpt-nano':     dict(n_layer=3, n_head=3, n_embd=48, n_encoder_layer=config.n_encoder_layer),
            }[config.model_type])
